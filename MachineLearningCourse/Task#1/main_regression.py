...
def preProcess(data):
    data = data.dropna(subset=['uniq_id', 'average_rating', 'product_name', 'price'])
    print(data.shape)
    print(data.isna().sum())
    data['number_available_in_stock'] = data['number_available_in_stock'].str.replace('new', '')
    data['average_rating'] = data['average_rating'].str.replace(' out of 5 stars', '')
...
Xtrain = model1.fit_transform(X_train)
poly_model = linear_model.LinearRegression()
start = time.time()
poly_model.fit(Xtrain, Y_train)
stop = time.time()
Y_train_pred = poly_model.predict(Xtrain)
print('Model #1 Training Time: ', stop - start, ' seconds')
print('Model #1 Training MSE: ', metrics.mean_squared_error(Y_train, Y_train_pred))
prediction = poly_model.predict(model1.fit_transform(X_test))
print('Model #1 Testing MSE: ', metrics.mean_squared_error(Y_test, prediction))
...
print('Model #2 Training Time: ', stop - start, 'seconds')
print('Model #2 Training MSE: ', metrics.mean_squared_error(Y_train, prediction))
prediction = model2.predict((X_test))
print('Model #2 Testing MSE: ', metrics.mean_squared_error(Y_test, prediction))